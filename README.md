...
| 이름 | 역할 |
|------|------|
| 정규원 (팀장) | 메인페이지 프론트엔드, 마법그림판·시간초 그림판 기능 구현, 관리자 페이지 프론트엔드, 전체 디테일 수정 및 디버깅, 배포 환경 구축 (EC2/Nginx/GitHub Actions CI/CD) |
| 김상준 | 갤러리 페이지, 유저 프로필·알림 페이지, 출석체크 이벤트 페이지, 관리자 페이지 디버깅, Cloudinary 연동 |
| 박진희 | ADMIN 페이지, 문의 페이지, 에러 페이지, 이용약관 페이지, 법적고지사항 페이지 |
| 이송아 | 결제 페이지, 데칼코마니, USER 관련 백엔드 작업 및 API 연동, ADMIN 배너관리 페이지, Git 관리 |
